# ============================================
# features/lgbm_selection.py
# LightGBM feature selection
# Logic aligned with xgb_selection.py / rf_selection.py
# ============================================
...
def train_one_direction_lgbm(
    ...
    """
    Train one-direction LightGBM model and select features.
    ...
    LGBMClassifier instead of XGBClassifier / RandomForestClassifier.
    """

    print(
        f"\n========== Start training {direction_name} "
        f"(LGBM), target = {target_col} ==========\n"
    )
    ...
    # ============================================
    # 2. Convert max_features into LightGBM
    ...
    if isinstance(max_features, str):
        ...
        feature_fraction = float(max_features)

    feature_fraction = float(
        np.clip(feature_fraction, 1e-6, 1.0)
    )
    ...
    def build_model(
        ...
        return LGBMClassifier(
            ...
            min_child_samples=int(min_samples_leaf),
    ...
    # ============================================
    # 8. Built-in LightGBM importance
    # ============================================
    gini_series = (
        pd.Series(
    ...
    if (
        ...
        for col in single_check_features:
            ...
            for w, (tr0, tr1, va0, va1) in enumerate(windows, 1):
                ...
                try:
                    single_lgbm.fit(X_tr_w, y_tr_w)

                    proba = single_lgbm.predict_proba(X_va_w)
